...
- 初始化 Git 版本控制准备工作。
- 按代码审查清单完成稳定性与安全边界修复：
  - 文件下载接口改为鉴权访问，支持 Header token 和 query token。
  - 设备列表不再返回授权 token，内容 API/WebSocket 不再返回本机文件绝对路径。
  - 上传改为磁盘临时文件 + 安全文件名 + 同名自动加序号，避免大文件进入内存。
  - 待审批设备队列改为按设备 ID 去重，保存真实申请时间，断开和超时自动清理。
  - Electron 打开文件夹 IPC 改为固定上传目录，不接受渲染进程传入路径。
  - Service Worker 不再缓存 `/api/` 响应。
  - 状态栏、图片预览和二维码渲染减少 `innerHTML` 使用。
  - `start-electron.bat` 改为相对路径，开机自启默认关闭。
- 清理项目目录中的临时文件、打包产物和本地运行数据，仅保留源码、文档、依赖目录和必要占位文件。
- 新增局域网电脑互联同步：
  - 通过 mDNS 发现同网 ClipSync 电脑并自动发起 WebSocket 连接。
  - 使用双向审批和远程 token 保存实现电脑间授权。
  - 新增 `peer_connections` 表、`/api/peers` 和 `/api/peers/:id/reconnect`。
  - 远程文字、图片、文件按原始 itemId 同步到本机，文件复制到本机数据目录。
  - 重连后自动补齐 24 小时内未过期内容，避免电脑离线期间漏收。
  - 设备管理面板增加“局域网电脑”状态区。
