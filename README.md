...
- Windows 托盘运行和开机自启
- PWA 资源支持，移动端可添加到主屏幕
- 多台 Windows 电脑局域网自动发现、双向审批和文件互传
...
1. 两台 Windows 电脑安装并启动 ClipSync。
2. 确保它们在同一个局域网内，并允许 TCP 服务端口和 UDP 5353。
3. 首次发现时，两台电脑会互相发起连接请求，分别在对方电脑上点击接受。
4. 审批通过后，文字、图片和文件会自动同步；离线期间错过的 24 小时内内容会在重连后补齐。
